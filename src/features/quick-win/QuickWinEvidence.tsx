...
import {
  deactivateQuickWinEvidence,
  fetchQuickWinEvidence,
  uploadQuickWinEvidence,
  type QuickWinEvidenceWithUrl,
} from './quickWinEvidenceApi'
import '../../components/ui/ActionPlanEvidence.css'

interface QuickWinEvidenceProps {
  candidateId: string
...
/** Evidencia (fotos, PDF) del win elegido — mismo componente visual que
 * ActionPlanEvidence (reutiliza su CSS), apuntando al bucket y tabla
...
export function QuickWinEvidence({ candidateId, organizationId, uploadedBy, canRemove }: QuickWinEvidenceProps) {
  ...
  async function reload() {
    const data = await fetchQuickWinEvidence(candidateId)
    setItems(data)
  }
  ...
  async function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    setError(null)
    try {
      await uploadQuickWinEvidence({ organizationId, candidateId, file, uploadedBy })
      await reload()
    } catch (err) {
  ...
  async function handleRemove(id: string) {
    ...
    setError(null)
    try {
      await deactivateQuickWinEvidence(id)
      await reload()
    } catch (err) {
